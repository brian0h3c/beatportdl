...
- **Downloads auto-organize into a `downloads/` folder.** If `downloads_directory` isn't set, BeatportDL creates a `downloads/` folder where you run it, and (with `sort_by_context`, now **on by default**) puts each download into its own subfolder named after the playlist, release, chart, label or artist — so a playlist lands in `./downloads/Playlist Name/`. Set `downloads_directory` to change the base folder, or `sort_by_context: false` for a flat dump into one folder.
...
| `downloads_directory`         |                                           | String     | Location for the downloads directory (defaults to a `downloads` folder in the current directory)                                                                                                                                                      |
| `sort_by_context`             | true                                      | Boolean    | Create a directory for each release, playlist, chart, label, or artist (on by default in this fork)                                                                                                                    |
...
| `playlist_directory_template` | {name}                                    | String     | Playlist directory template                                                                                                                                                               |
