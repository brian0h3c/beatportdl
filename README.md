...
- **Downloads auto-organize into named folders.** If `downloads_directory` isn't set, BeatportDL saves into the folder you run it from, and (with `sort_by_context`, now **on by default**) puts each download into its own subfolder named after the playlist, release, chart, label or artist — so a playlist lands in `./Playlist Name/`. Set `downloads_directory` to change the base folder, or `sort_by_context: false` for a flat dump into one folder.
